...
var CheckoutAddress = /*#__PURE__*/function () {
  ...
  _createClass(CheckoutAddress, [{
    ...
    value: function init() {
      ...
      $(document).on('click', '#billing_address_same_as_shipping_address', function (event) {
        if ($(event.currentTarget).is(':checked')) {
...
var MainCheckout = /*#__PURE__*/function () {
  ...
  _createClass(MainCheckout, [{
    ...
    value: function init() {
      ...
      var customerShippingAddressForm = '.customer-address-payment-form';
      var customerBillingAddressForm = '.customer-billing-address-form';
      ...
      var reloadAddressForm = function reloadAddressForm(url) {
        var isAddressAvailable = $(customerShippingAddressForm + ' #address_id option:selected').val();
        var addressForm = $(customerShippingAddressForm).clone();
        var billingAddressForm = $(customerBillingAddressForm).clone();
        var selectedCountry = $(customerShippingAddressForm + ' #address_country option:selected').val();
        var selectedState = $(customerShippingAddressForm + ' #address_state option:selected').val();
        var selectedCity = $(customerShippingAddressForm + ' #address_city option:selected').val();
        var billingAddressSelectedCountry = $(customerBillingAddressForm + ' #billing-address-country option:selected').val();
        var billingAddressSelectedState = $(customerBillingAddressForm + ' #billing-address-state option:selected').val();
        var billingAddressSelectedCity = $(customerBillingAddressForm + ' #billing-address-city option:selected').val();
        var billingAddressId = $(customerBillingAddressForm + ' #billing_address_id option:selected').val();
        $('.shipping-info-loading').show();
        $(shippingForm).load(url, function () {
      ...
      var validatedFormFields = function validatedFormFields() {
        if ($(shippingForm).find('.shipping_method_input').length && !$(shippingForm).find('.shipping_method_input:checked').val()) {
          return false;
        }
      ...
      $(document).on('change', customerShippingAddressForm + ' .address-control-item', function (event) {
        var _self = $(event.currentTarget);
      ...
      $(document).on('change', customerBillingAddressForm + ' .address-control-item', function (event) {
        var _self = $(event.currentTarget);
        var $form = _self.closest('form');
        var val = _self.find(':selected').val();
        if (val) {
    ...
    key: "handleError",
    value: function handleError(data, $container) {
    ...
    value: function handleValidationError(errors, $container) {
      var message = '';
      $.each(errors, function (index, item) {
        message += item + '<br />';
        var inputName = MainCheckout.dotArrayToJs(index);
